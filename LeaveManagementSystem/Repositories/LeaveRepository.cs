...
namespace LeaveManagementSystem.Repositories
{
    public class LeaveRepository : ILeaveRepository
    {
        ...
        public List<LeaveRequestModel> SearchLeaves(
    ...
    int? employeeId)
        {
            ...
            ht.Add(
                "EmployeeNames",
                employeeNames != null && employeeNames.Any()
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    return new LeaveRequestModel
                    {
                        ...
                        Status = row["Status"].ToString(),
                        AppliedDate = Convert.ToDateTime(row["AppliedDate"]),
                        ApprovedByName = row["ApprovedByName"] == DBNull.Value
                                            ? string.Empty
        ...
        public DashboardModel GetDashboardCounts(string role,int employeeId)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    PendingCount = row["PendingCount"] != DBNull.Value
                        ? Convert.ToInt32(row["PendingCount"])
                        : 0,

                    ApprovedCount = row["ApprovedCount"] != DBNull.Value
                        ? Convert.ToInt32(row["ApprovedCount"])
                        : 0,

                    RejectedCount = row["RejectedCount"] != DBNull.Value
                        ? Convert.ToInt32(row["RejectedCount"])
                        : 0,

                    MonthlyCount = row["MonthlyCount"] != DBNull.Value
        ...
        public DashboardModel GetLeaveSummary(int employeeId)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                return new DashboardModel
                {
                    PendingLeaves = row["PendingLeaves"] != DBNull.Value
                                        ? Convert.ToInt32(row["PendingLeaves"])
                                        : 0,

                    ApprovedLeaves = row["ApprovedLeaves"] != DBNull.Value
                                        ? Convert.ToInt32(row["ApprovedLeaves"])
                                        : 0,

                    RejectedLeaves = row["RejectedLeaves"] != DBNull.Value
                                        ? Convert.ToInt32(row["RejectedLeaves"])
                                        : 0,

                    TotalDaysTaken = row["TotalDaysTaken"] != DBNull.Value
                                        ? Convert.ToInt32(row["TotalDaysTaken"])
                                        : 0
                };
            }
